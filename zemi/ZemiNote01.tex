...
\subsection{クーロン相互作用}
    ...
    \end{align}
    となる。第二項目は背景の正電荷から受ける斥力相互作用で第三項目は背景の正電荷のエネルギーを電子のエネルギーに上乗せして表している項である。\\
    ここで$Z$は正電荷の価数、$\rho_0$は正電荷密度を表し、空間に一様に分布しているとした。また電気的中性条件$V\rho_0 Ze = e N$を満たす。\\
    ...
    \begin{derivationBox}
        ...
        となる。
    \end{derivationBox}
    クーロン相互作用が長距離まで及ぶために$U_{\bm{q}=0}$は発散してしまうが形式的に$U_{\bm{q}=0}$のまま残している。\\
    ...
    \end{align}
    となり、ここでは電子数が$N$に固定されたモデルを考えるので$\hat{\mathscr{N}}$は保存料$N$に置き換えてよい、従って$\rho_0 = N/ZV$に注意すると
    \begin{align}
        &=\frac{1}{2V} U_{\bm{q}=0} (N^2-N) - U_{\bm{q}=0}N Z\rho_0 + \frac{V}{2}U_{\bm{q}=0}(Z\rho_0)^2 \notag \\
        &=\frac{1}{2V} U_{\bm{q}=0} (N^2-N) - U_{\bm{q}=0}N Z\frac{N}{VZ} + \frac{V}{2}U_{\bm{q}=0}\left(Z\frac{N}{ZV}\right)^2 \\
        &=\frac{1}{2V} U_{\bm{q}=0} (N^2-N) - U_{\bm{q}=0}\frac{N^2}{V} + \frac{1}{2}U_{\bm{q}=0}\frac{N^2}{V} \\
        &= -\frac{1}{2}\frac{N}{V}U_{\bm{q}=0}
    \end{align}
