...
\subsection{第二量子化の定式化}
    ...
    \end{derivationBox}
\subsection{物理量の期待値}
    系のエネルギー$\hat{\mathscr{H}}$の期待値は、波動関数を用いて
    \begin{align}
        \int d\bm{r}_1 \cdots d\bm{r}_N\ \Psi_{\text{AS}}^* (\bm{r}_1,\cdots,\bm{r}_N) \hat{\mathscr{H}}\Psi_{\text{AS}} (\bm{r}_1,\cdots,\bm{r}_N)
    \end{align}
    で表される。前節で導入した波動関数の状態ベクトルを用いて、この期待値がどのように表されるのかを考える
    \begin{formula}[エネルギーの期待値]
        \begin{equation}
            \braket{\hat{\mathscr{H}}} = \braket{\Psi_N(t)|\hat{\mathscr{H}}|\Psi_N(t)}
        \end{equation}
    \end{formula}
    \begin{derivationBox}
        \begin{align}
            \braket{\hat{\mathscr{H}}} &= \int d\bm{r}_1 \cdots d\bm{r}_N\ \Psi_{\text{AS}}^* (\bm{r}_1,\cdots,\bm{r}_N) \hat{\mathscr{H}}\Psi_{\text{AS}} (\bm{r}_1,\cdots,\bm{r}_N) \notag \\
            &= 
        \end{align}
    \end{derivationBox}
\end{document}